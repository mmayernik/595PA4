...
void post_order_traversal(Node * root, double * array, int * idx, double * min, double * max){
	...
	if(!((root -> parallel == 0) && (root -> left == NULL) && (root -> right == NULL)) || (root -> parallel > 0)){	
		post_order_traversal(root->left, array, idx,min,max);
		post_order_traversal(root->right, array, idx,min,max);
	} else {
		if(*max < (root->t + array[*idx  ])){
			*max = root->t + array[*idx];
		} 

		if(*min > (root->t + array[*idx  ])){
			*min = root->t + array[*idx];
		} 
...
int print_spice_netlist(FILE* print_file, Node * root, char *  input_label, double length_to_parent, double r_unit, double c_unit, int * size){
	...
	//Current node must be a joint
	  ...
	  if(print_spice_netlist(print_file, root->left, pass_label,root->wire_l, r_unit, c_unit,size) != 0){
			return -1;
		}

		if(root->wire_r != -1.0f){
		  if(print_spice_netlist(print_file, root->right, pass_label,root->wire_r, r_unit, c_unit,size) != 0){
				return -1;
			}
		}
